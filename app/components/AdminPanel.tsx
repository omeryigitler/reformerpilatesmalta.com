...
const convertDisplayDateToIso = (date: string) => {
...
export const AdminPanel = ({
    ...
    const handleAddFutureBookingForMember = async () => {
        if (!selectedMember || futureSlotDate.length !== 10 || futureSlotTime.length !== 5) return;
        ...
        const dateStr = convertDisplayDateToIso(futureSlotDate);
        ...
        const slot: Slot = {
            ...
            time: futureSlotTime,
            status: 'Booked',
            bookedBy: `${selectedMember.firstName} ${selectedMember.lastName} (Admin)`,
            bookedByEmail: selectedMember.email.trim().toLowerCase()
        };

        try {
            setIsSavingFutureBooking(true);
            await setDoc(doc(db, 'slots', slotId), slot);
            await sendUserBookingConfirmation(selectedMember, slot);
            showNotification(`Future class booked for ${selectedMember.firstName} on ${formatDateDisplay(dateStr)} at ${futureSlotTime}.`, 'success');
            setFutureSlotDate('');
            setFutureSlotTime('');
            setIsFutureBookingOpen(false);
        } catch (error) {
    ...
    const [isFutureBookingOpen, setIsFutureBookingOpen] = useState(false);
                                                        ...
                                                        <div key={idx} className="flex flex-col sm:flex-row items-center justify-between p-4 bg-gray-50 rounded-2xl hover:bg-white hover:shadow-md transition-all border border-gray-100 hover:border-[#CE8E94]/30 group">
                                                            ...
                                                            <div className="flex items-center gap-4 w-full sm:w-auto">
                                                                ...
                                                                <div className="flex flex-col items-center sm:items-start flex-1">
                                                                    ...
                                                                    <div className="flex items-center gap-1.5 max-w-[180px] mt-1">
                                                                        {slot.bookedBy ? (
                                                                            <div className="flex items-center gap-1.5">
                    ...
                    <Modal onClose={() => {
                        ...
                        <div className="space-y-4 md:space-y-5 pr-1">
                            ...
                            <div className="grid grid-cols-4 gap-2 px-1">
                                ...
                                <div
                                    onClick={() => {
                                        setIsFutureBookingOpen(!isFutureBookingOpen);
                                        setIsPastBookingOpen(false);
                                    }}
                            ...
                            {/* --- ADD FUTURE BOOKING ACCORDION --- */}
                            {isFutureBookingOpen && (
                                <div className="bg-[#CE8E94]/5 p-3 rounded-xl space-y-3 animate-in slide-in-from-top-2 fade-in duration-200 border border-[#CE8E94]/15">
                                    <div className="grid grid-cols-2 gap-2">
                                        <div>
                                            <label className="text-[10px] font-bold text-[#CE8E94] uppercase mb-1 block">Date (DD/MM/YYYY)</label>
                                            <input
                                                type="text"
                                                placeholder="(DD/MM/YYYY)"
                                                value={futureSlotDate}
                                                onChange={e => setFutureSlotDate(formatMaskedDate(e.target.value))}
                                                className="w-full p-2 border border-[#CE8E94]/20 rounded-lg focus:ring-2 focus:ring-[#CE8E94] outline-none text-sm font-sans font-bold text-gray-700 placeholder:font-normal placeholder:text-gray-400 bg-white"
                                            />
                                        </div>
                                        <div>
                                            <label className="text-[10px] font-bold text-[#CE8E94] uppercase mb-1 block">Time (HH:MM)</label>
                                            <input
                                                type="text"
                                                placeholder="(HH:MM)"
                                                value={futureSlotTime}
                                                onChange={e => setFutureSlotTime(formatMaskedTime(e.target.value))}
                                                className="w-full p-2 border border-[#CE8E94]/20 rounded-lg focus:ring-2 focus:ring-[#CE8E94] outline-none text-sm font-sans font-bold text-gray-700 placeholder:font-normal placeholder:text-gray-400 bg-white"
                                            />
                                        </div>
                                    </div>
                                    <Button
                                        className="w-full h-9 bg-[#CE8E94] text-white hover:bg-[#b07278] text-xs rounded-lg disabled:opacity-50 disabled:cursor-not-allowed"
                            ...
                            <div className="pt-4 border-t border-gray-100">
                                <button
                                    onClick={() => {
                                        setIsPastBookingOpen(!isPastBookingOpen);
                                        setIsFutureBookingOpen(false);
                                    }}
