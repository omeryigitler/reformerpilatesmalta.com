...
/**
 * EXPERT-REVISED SINGLE PIECE DESIGN
 * Captures background + foreground as ONE crisp image to fix Instagram missing stickers.
 * SAFE AREA padding included to ensure content is centered across device ratios.
 */
const BadgeStoryContent = ({ title, icon, description }: { title: string, icon: React.ReactNode, description: string }) => {
    return (
        ...
        >
            ...
            {/* 2. Content Layer (Top) - Respecting Safe Areas with padding */}
            <div className="flex flex-col items-center justify-center w-full h-full py-[240px]">
                {/* Main Achievement Card */}
                <div
                    className="w-[700px] aspect-[1/1.4] flex flex-col items-center justify-between p-24 bg-[#FEF9F9] rounded-[220px] shadow-[0_60px_120px_-30px_rgba(206,142,148,0.3)] border border-white/20"
                >
                    {/* Glossy Inner Layer */}
                    <div className="w-full flex-grow flex flex-col items-center justify-between bg-white rounded-[180px] p-16 shadow-[inset_0_2px_10px_rgba(255,255,255,0.8),0_40px_80px_-15px_rgba(206,142,148,0.2)] my-6">
                        <div className="flex-1 flex flex-col items-center justify-center w-full mt-10">
                            {/* Icon Slot */}
                            <div className="text-[#CE8E94] mb-12" style={{ filter: 'drop-shadow(0 15px 25px rgba(206,142,148,0.3))' }}>
                                {title === 'SOLARIS' ? (
...
export const ShareModal = ({ isOpen, onClose, achievementTitle, achievementIcon, achievementDescription }: ShareModalProps) => {
    ...
    const generateImageBlob = async (): Promise<Blob | null> => {
        ...
        try {
            // EXPERT SUGGESTION: Ensure fonts are loaded and use higher pixelRatio
            ...
            console.error('Expert capture failed:', err);
            return null;
        }
    };

    // Pre-calculate on opening for smooth share
    React.useEffect(() => {
        if (isOpen) {
    ...
    const handleAction = async (platform: string) => {
        ...
        try {
            ...
            if (platform === 'Download Image') {
                ...
                // EXPERT SUGGESTION: iOS-specific robust flow
                const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent);

                if (isIOS && platform === 'Instagram') {
                    setAlertConfig({
                        isOpen: true,
                        title: 'Ready for Instagram!',
                        message: 'The HD image is being prepared. Please save it to your gallery, then open Instagram to share your success!',
                        type: 'info'
                    });
                    triggerDownload(blob, filename);
                    setTimeout(() => {
            ...
            console.error('Expert action failure:', err);
        } finally {
        ...
        <>
            <Modal onClose={onClose} className="max-w-[420px]" useDefaultPadding={false}>
                <div className="text-center pt-8 pb-4 px-4 overflow-hidden">
                    ...
                    {/* COMPACT UI PREVIEW - PERFECTLY SCALED */}
            ...
            {/* EXPERT-OPTIMIZED CAPTURE - Standard position to ensure browser paint */}
            <div
                id="master-capture-container"
                className="absolute"
                style={{ width: '1080px', height: '1920px', left: '-5000px', top: '0', pointerEvents: 'none' }}
            >
                <BadgeStoryContent title={achievementTitle} icon={achievementIcon} description={achievementDescription} />
            </div>
